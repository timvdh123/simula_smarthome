...
def predictive_score_metrics(dataX, dataX_hat):
    ...
    def predictor(X, T):
        ...
        with tf.variable_scope("predictor", reuse=tf.AUTO_REUSE) as vs:
            ...
            d_outputs, d_last_states = tf.nn.dynamic_rnn(d_cell, X, dtype=tf.float32,
                                                         sequence_length=T)
    ...
    for itt in range(iterations):
        ...
        Y_mb = list(
            np.reshape(dataX_hat[i][1:, (data_dim - 1)], [len(dataX_hat[i][1:, (data_dim - 1)]), 1])
            for i in train_idx)
    ...
    T_mb = list(dataT[i] - 1 for i in train_idx)
    Y_mb = list(
        np.reshape(dataX[i][1:, (data_dim - 1)], [len(dataX[i][1:, (data_dim - 1)]), 1]) for i in
        train_idx)
