...
def tgan(dataX, parameters):
    ...
    def rnn_cell(module_name):
        ...
        # LSTM Layer Normalization
            ...
            rnn_cell = tf.contrib.rnn.LayerNormBasicLSTMCell(num_units=hidden_dim,
    ...
    def embedder(X, T):
        ...
        with tf.variable_scope("embedder", reuse=tf.AUTO_REUSE):
            ...
            e_outputs, e_last_states = tf.nn.dynamic_rnn(e_cell, X, dtype=tf.float32,
                                                         sequence_length=T)

            H = tf.contrib.layers.fully_connected(e_outputs, hidden_dim,
    ...
    def recovery(H, T):
        ...
        with tf.variable_scope("recovery", reuse=tf.AUTO_REUSE):
            ...
            r_outputs, r_last_states = tf.nn.dynamic_rnn(r_cell, H, dtype=tf.float32,
                                                         sequence_length=T)

            X_tilde = tf.contrib.layers.fully_connected(r_outputs, data_dim,
                                                        activation_fn=tf.nn.sigmoid)
    ...
    def generator(Z, T):
        ...
        with tf.variable_scope("generator", reuse=tf.AUTO_REUSE):
            ...
            e_outputs, e_last_states = tf.nn.dynamic_rnn(e_cell, Z, dtype=tf.float32,
                                                         sequence_length=T)

            E = tf.contrib.layers.fully_connected(e_outputs, hidden_dim,
                                                  activation_fn=tf.nn.sigmoid)
    ...
    def supervisor(H, T):
        ...
        with tf.variable_scope("supervisor", reuse=tf.AUTO_REUSE):
            e_cell = tf.nn.rnn_cell.MultiRNNCell(
                [rnn_cell(module_name) for _ in range(num_layers - 1)])

            e_outputs, e_last_states = tf.nn.dynamic_rnn(e_cell, H, dtype=tf.float32,
                                                         sequence_length=T)

            S = tf.contrib.layers.fully_connected(e_outputs, hidden_dim,
                                                  activation_fn=tf.nn.sigmoid)
    ...
    def discriminator(H, T):
        ...
        with tf.variable_scope("discriminator", reuse=tf.AUTO_REUSE):
            ...
            d_outputs, d_last_states = tf.nn.dynamic_rnn(d_cell, H, dtype=tf.float32,
                                                         sequence_length=T)
    ...
    # 3. Two Momments
    G_loss_V1 = tf.reduce_mean(np.abs(
        tf.sqrt(tf.nn.moments(X_hat, [0])[1] + 1e-6) - tf.sqrt(tf.nn.moments(X, [0])[1] + 1e-6)))
...
if __name__ == '__main__':
    ...
    series_sensor_data = generate_data(combined1, 600, 144, 6)

    parameters = dict()

    parameters['hidden_dim'] = len(series_sensor_data[0][0, :]) * 4
    parameters['num_layers'] = 3
    parameters['iterations'] = 10000
    parameters['batch_size'] = 128
    parameters['module_name'] = 'lstm'  # Other options: 'lstm' or 'lstmLN'
    parameters['z_dim'] = len(series_sensor_data[0][0, :])
    ...
    for it in range(Iteration):
        ...
        dataX_hat = tgan(series_sensor_data, parameters)
        ...
        for tt in range(Sub_Iteration):
            Temp_Disc = discriminative_score_metrics(series_sensor_data, dataX_hat)
